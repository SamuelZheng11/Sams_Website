...
import { Person, setPeople, setDebts, setAmount, setName, Debt } from './slice/SettlementSlice';
...
export default function SettlementCalculator() {
	...
	const onSubmit = (fullName: string, amount: string, id?: number) => {
		if (isNaN(parseInt(amount)) || parseInt(amount).toString().length !== amount.length) {alert('Amount paid must be a number'); return;}
	...
	const removePerson = (id: number) => {
		...
		const newListOfPeopleWithIdsUpdated = newListOfPeople.map<Person>((person, i) => {return {...person, id: i + 1}});
		...
		return (
			<div key={person.id} className="settlement-calculator-person">
				...
				<div className='settlement-calculator-person-button-contanier'>
					<IconButton className='settlement-calculator-person-edit-button' onClick={() => {editPerson(person.id, person.fullName, person.amount)}}>
						<EditIcon />
					</IconButton>
					<IconButton className='settlement-calculator-person-delete-button' onClick={() => removePerson(person.id)}>
						<DeleteIcon />
					</IconButton>
				</div>
			</div>
	...
	return (
		<div className="settlement-calculator">
			...
			{people?.length ? (<div>{people.map((person: Person) => PersonList(person))}</div>) : (<div className="settlement-calculator-person">Add a person to begin</div>)}
			<div className="settlement-calculator-buttons-container">
				<Modal open={modalOpen} modalButtonVariant='outlined' buttonText='Add person' submit cancel submitText={isEditing ? 'Save' : 'Add'} onSubmit={() => onSubmit(modalPersonName, modalPersonAmount, modalPersonId)} onClose={() => setModalOpen(false)} onOpen={() => setModalOpen(true)}>
					<div className="modal-person-input">
						<div className="modal-person-input-field">
							<span>Name: </span>
							<input value={modalPersonName} placeholder="Enter a name" onChange={(next) => setModalPersonName(next.target.value)}></input>
						</div>
						<div className="modal-person-input-field">
							<span>Amount paid: </span>
							<input placeholder="Enter an amount" value={modalPersonAmount} onChange={(next) => setModalPersonAmount(next.target.value)}></input>
						</div>
					</div>
				</Modal>
			...
			{people?.length && (debts !== undefined && (debts.length !== 0 ? debts.map((debt: Debt, i) => (
